...
import { FormEvent, Ref, forwardRef, useEffect } from "react";
...
function SignupForm({ ...props }: HTMLMotionProps<"form">, ref: Ref<HTMLFormElement>) {
  ...
  return (
    <motion.form onSubmit={handleSignUp} ref={ref} {...props}>
      ...
      <InputField disabled={isLoading} id="signupUsername" name="signupUsername" label="username" type="text" />
      <InputField disabled={isLoading} id="signupEmail" name="signupEmail" label="email" type="email" />
      <InputField disabled={isLoading} id="signupPassword" name="signupPassword" label="password" type="password" />
      <InputField
        disabled={isLoading}
