...
export default function LeaderboardTable() {
  ...
  return (
    <table className="w-full text-center">
      <thead>
        <tr>
          <th className="sticky top-0 border-b-2 border-accent bg-fill-2 p-4 text-start capitalize">#</th>
      ...
      <tbody>
        {data.map((item: any, i: number) => (
          <tr
            key={item._id}
            className={`bg-opacity-50 hover:bg-opacity-100 ${userAuth?._id === item._id ? "bg-accent" : "bg-fill-1"}`}
          >
