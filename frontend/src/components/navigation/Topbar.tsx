...
export default function Topbar() {
  ...
  const toggleDarkMode = usePreferencesPersistedStore((store) => store.toggleDarkMode);
  ...
  return (
    <div className="flex items-center gap-4 border-b border-fill-2 bg-fill-3 p-2">
      ...
      <div className="relative">
        <button className="rounded-sm outline-none focus:outline-accent" onClick={() => setIsOpen((prev) => !prev)}>
          <Translate size="35" />
        </button>
        <AnimatePresence>
          {isOpen && (
